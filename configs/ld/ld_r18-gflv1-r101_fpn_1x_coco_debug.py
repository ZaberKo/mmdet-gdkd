...
custom_imports = dict(
    imports=['distiller.dense_heads.ld_head'], allow_failed_imports=False)
...
_base_.wandb_backend.init_kwargs.update(
    dict(
        ...
        tags=['ld', 'orikd', 'r18-gflv1-r101', 'fpn', '1x', 'coco']
    )
)
...
log_processor = dict(
    custom_cfg=[
        dict(
            data_src="bbox_avg_fractor",
            log_name="bbox_avg_fractor_mean",
            method_name="mean",
            window_size='global'
        ),
        dict(
            data_src="ld_real_avg_fractor_ratio",
            log_name="ld_real_avg_fractor_ratio_mean",
            method_name="mean",
            window_size='global'
        )
    ]
